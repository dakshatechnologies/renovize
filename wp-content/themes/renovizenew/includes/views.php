...
class VA_Listing_Renew extends VA_Listing_Edit {
	...
	function template_redirect() {
		...
		if ( $va_options->form_val_lang ) {
			$lang_code = trim( $va_options->form_val_lang );
			wp_enqueue_script( 
				'validate-lang',
				get_template_directory_uri() . "/scripts/validate/localization/messages_$lang_code.js",
...
class VA_Listing_Claim extends APP_View {
	...
	function parse_query( $wp_query ) {
		...
		if ( empty( $claimable ) ) {
			if ( get_current_user_id() != get_post_meta( $listing_id, 'claimee', true ) ) // During a 'un-moderated claim' checkout step, the claim will have been approved and 'claimee', which will equal get_current_user_id(), will be present in place of listing_claimable
