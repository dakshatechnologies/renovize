...
class VA_Widget_Create_Listing_Button extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'create_listing_button', __( 'renovize - Create Listing Button', APP_TD ), $widget_ops );
	}
...
class VA_Widget_Listings_Events_Map extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'listings_events_map', __( 'renovize - Multiple Listings/Events Map', APP_TD ), $widget_ops );
	}

	function widget( $args, $instance ) {

		if ( is_singular( VA_LISTING_PTYPE ) || ( va_events_enabled() && is_singular( VA_EVENT_PTYPE ) ) ) {
			appthemes_display_notice( 'error', __( 'The \'renovize - Multiple Listing/Event Map\' widget must only be used on the \'List Page Top Sidebar\'', APP_TD ) );
			return;
		}
...
class VA_Widget_Listing_Map extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'listing_event_map', __( 'renovize - Single Listing/Event Map', APP_TD ), $widget_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( !is_singular( VA_LISTING_PTYPE ) && ( va_events_enabled() && !is_singular( VA_EVENT_PTYPE ) ) ) {
			...
			appthemes_display_notice( 'error', __( 'The \'renovize - Single Listing/Event Map\' widget must only be used on the \'Single Listing Sidebar\' or \'Single Event Sidebar\'', APP_TD ) );
			echo '</div>';
			return;
		}
		...
		if ( $directions ) {
			...
			$html = html( 'label', array(), __('Get Directions From: ' , APP_TD ) . html( 'br' , array() ) . html( 'input', array( 'type' => 'text' , 'id' => 'directions_from' ) ) );
...
class VA_Widget_Categories extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'listing_categories', __( 'renovize - Related Categories', APP_TD ), $widget_ops );
	}
...
class VA_Widget_Recent_Reviews extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'recent_reviews', __( 'renovize - Recent Reviews', APP_TD ), $widget_ops );
...
class VA_Widget_Recent_Listings extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'recent_listings', __( 'renovize - Recent Listings', APP_TD ), $widget_ops );
...
class VA_Widget_Connect extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'connect', __( 'renovize - Connect', APP_TD ), $widget_ops );
	}
...
class VA_Widget_Sidebar_Ad extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'sidebar_ad', __( 'renovize - Sidebar Ad', APP_TD ), $widget_ops );
	}
...
class VA_Widget_Listings_Ad extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'listings_ad', __( 'renovize - 468x60 ad banner', APP_TD ), $widget_ops );
	}
...
class VA_Widget_Popular_Categories extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'popular_listing_categories', __( 'renovize - Popular Categories', APP_TD ), $widget_ops );
	}
