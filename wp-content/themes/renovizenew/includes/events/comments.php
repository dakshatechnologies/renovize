...
function va_event_comment( $comment, $args, $depth ) {
	...
	if( !empty( $comment->user_id ) ) {
		...
		$user_url = va_dashboard_url( 'event-comments', $user->ID, ( $user->ID == $comment->user_id ) );
	}
?>
	<div <?php comment_class('comment'); ?> id="comment-<?php echo $comment->comment_ID; ?>">
		<div class="comment-inner">
			<div class="comment-meta">
				<div class="comment-author">
					...
					<ul class="comment-author-meta">
						...
						<li><?php echo html_link( va_get_event_comment_link( $comment->comment_ID ), '#' ); ?></li>
					</ul>
					<?php 
					} else { 
			...
			<div class="comment-content">
				<p class="comment-date"><?php echo mysql2date( get_option('date_format'), $comment->comment_date ); ?></p>
				<div class="clear"></div>
...
function va_fix_event_comment_template_comment_query( $wp_comment_query ) {
	...
	if ( isset( $post->post_type ) && VA_EVENT_PTYPE == $post->post_type ) {
