...
function get_the_event_address( $event_id = '' ) {
	...
	if ( !empty( $location ) ) {
		if ( !empty( $location_url ) ) {
			$html .= html( 'div', array( 'class' => 'location location-link' ), html( 'a', array( 'href' => esc_url( 'http://' . $location_url ), 'target' => '_blank' ), $location ) );
		} else {
...
function va_get_the_event_days_span( $event_id = 0, $date_format = '', $sep = ' - ' ) {
	...
	$span = $va_locale->date( $date_format, strtotime( reset( $event_days )->slug ) );
	$span .= $sep;
	$span .= $va_locale->date( $date_format, strtotime( end( $event_days )->slug ) );
